...
// Write account, plan, or window to the stage file after connecting the app.
...
config.accounts.codex[0]!.label = "검증용";
...
function ingest(stage: keyof typeof states) {
  const { minutes, ...context } = states[stage];
  const now = Date.now();
  const observation: QuotaObservation = {
    provider: "codex", account: "default", bucket: `codex:primary:${minutes}`,
  ...
  service.collection.recordAttempt("codex", "default", "codex-app-server", now, null, null);
...
setInterval(async () => {
  ...
  try {
    ...
    if (stage !== previous && ["account", "plan", "window"].includes(stage)) {
      previous = stage;
      const events = ingest(stage as keyof typeof states);
